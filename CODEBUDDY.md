...
| `vla_env/scripts/demo_human.py` | M11.5 | `HUMAN_DEMO_OK`（人类式按键录制：`--task dig_stone/dig_dirt/kill_animal/place_dirt`、`--replay <ep>` 种子回放；输出 frame↔按键对齐 canonical 数据 + mp4，DESIGN.md §17.4） |
...
- `server_grpc.py` — 全部 RPC 封装（reset_world/set_task/get_step_result/get_state/get_voxels/compute_path/teleport/spawn_entity/set_block…）。
...
- `tasks.py` — 任务知识**单一来源**（M11.5）：`PROFILES`/`TOOL_FOR_BLOCK`（方块→工具）/`KIT_TOOL_SLOT`/`SURVIVAL_KIT`（镐/剑/铲/泥土，hotbar 0-3）。新增任务 = 服务端加 tasks/*.json + 此处加 profile。
