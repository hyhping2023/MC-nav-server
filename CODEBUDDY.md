...
**单材质元世界 + 录制状态机**：每种地表材质有独立持久化存档 `server/vla_surface_<surface>/` 与 `world-meta.json`；gRPC `SelectSurfaceWorld` / Python `surface=` 选择世界。采集采用 `MOVE_TO_STAND(move_only) -> DIG_BATCH -> PILLAR`：导航不攻击、不跳、不挖穿；到合法站位后通过客户端准星距离门控批量挖可达目标；只在高处剩余目标不可从地面触及时脚底垫方块。使用说明见 `docs/demo_recording.md`。
...
| `vla_env/scripts/demo_human.py` | M11.5 | `HUMAN_DEMO_OK`（人类式按键录制：`--task dig_stone/dig_dirt/kill_animal/place_dirt/collect_wood`、`--setup-env`（M11.7 先削平世界+放物体再录制）、`--replay <ep>` 种子回放；输出 frame↔按键对齐 canonical 数据 + mp4，DESIGN.md §17.4） |
...
- `world/FlatWorldGen`（**M11.7 WIP**）— 以玩家为圆心削平世界到海平面（半径默认 64）：`y>sea` 清 AIR、`y==sea` 空格填 GRASS_BLOCK，全部 `setBlockData(data,false)` 关物理更新；大区域**分 tick 批处理**（每 tick ≤32k 方块、目标 100 tick 内完成，`runTaskLater` 链式调度）；`seed>=0` 削平完成后随机出生点并传送玩家（`Random(seed)` 可复现），`seed=-1` 保持当前出生点。**RPC 返回只代表已入队**，削平是异步完成的（demo_human 轮询 get_state 玩家位置变化判断完成）。
- `world/ObjectPlacer`（**M11.7 WIP**）— 在玩家周围 `[minDist,maxDist]` 环带放置树（OAK_LOG 主干 4-6 + 2×2×3 OAK_LEAVES 叶冠）/石柱/泥柱（1×1×4），`Random(seed)` 位置可复现 + 中心间隔 ≥4 去重 + 避开玩家脚下；小物体量单 tick 完成。
...
- `server_grpc.py` — 全部 RPC 封装（reset_world/set_task/get_step_result/get_state/get_voxels/compute_path/teleport/spawn_entity/set_block…；**M11.7** `flatten_world`/`place_objects`，注意这俩返回只代表服务端已入队）。
...
- `dataset/` — canonical 落盘（`schema.py` + `human_recorder.py`/`oracle_recorder.py`），M11.5 契约见 DESIGN.md §17.4。
...
12. **M11.7 FlattenWorld/PlaceObjects 是异步批处理**：gRPC 返回只代表已入队（主线程分 tick 削平，半径 64 需数秒）。seed≥0 时玩家位置变化 = 削平完成信号（demo_human `--setup-env` 用 get_state 轮询 20s 判断）；place_objects 必须在出生点落定后再调（圆心依赖新坐标）。
